...
!==========================================================================
    ...
    if (.not.allocated(nonlocal_correction)) allocate (nonlocal_correction(subgrid_dim(t_dim_index),n_source_index))
    ...
    if (EMEP_grid_interpolation_flag.ge.1) then
        ...
        write(unit_logfile,'(A)')'Calculating EMEP local subgrid contribution using area weighted interpolation'
        ...
        write(unit_logfile,*) 'Weighting grid dimensions and centres: ',n_weight,ii_w0,jj_w0
    
        allocate (weighting_nc(n_weight,n_weight,dim_length_nc(time_dim_nc_index),n_source_index)) !EMEP grid weighting for interpolation. Does not need a source index for area weighting
        allocate (area_weighting_nc(subgrid_dim(x_dim_index),subgrid_dim(y_dim_index),n_weight,n_weight,dim_length_nc(time_dim_nc_index),n_source_index)) !EMEP grid weighting for area interpolation
        ...
        if (use_subgrid(i,j,allsource_index)) then
            ...
            do ii=ii_start,ii_end
                ...
                if (lon_max.gt.lon_min.and.lat_max.gt.lat_min) then
                    weighting_nc(ii_w,jj_w,tt,:)=(lat_max-lat_min)*(lon_max-lon_min)/dgrid_nc(lon_nc_index)/dgrid_nc(lat_nc_index)/EMEP_grid_interpolation_size_sqr
                else
                    weighting_nc(ii_w,jj_w,tt,:)=0.
                endif                
                ...
                !else
                    subgrid(i,j,tt,emep_local_subgrid_index,:,emep_subsource)=subgrid(i,j,tt,emep_local_subgrid_index,:,emep_subsource) &
                        +var3d_nc(ii_nc,jj_nc,tt,local_nc_index,:)*weighting_nc(ii_w,jj_w,tt,:)
                    subgrid(i,j,tt,emep_nonlocal_subgrid_index,:,emep_subsource)=subgrid(i,j,tt,emep_nonlocal_subgrid_index,:,emep_subsource) &
                        +(var3d_nc(ii_nc,jj_nc,tt,conc_nc_index,:)-var3d_nc(ii_nc,jj_nc,tt,local_nc_index,:))*weighting_nc(ii_w,jj_w,tt,:)

                    !Interpolate the other EMEP compounds as well to subgrid
                    do i_comp=1,n_compound_loop
                        comp_EMEP_subgrid(i,j,tt,compound_loop_index(i_comp))=comp_EMEP_subgrid(i,j,tt,compound_loop_index(i_comp)) &
                        +comp_var3d_nc(ii_nc,jj_nc,tt,compound_loop_index(i_comp))*weighting_nc(ii_w,jj_w,tt,allsource_index)
                    enddo
            ...
            do ii=ii_start,ii_end
                ...
                if (jj.ne.0.or.ii.ne.0) then
                    ...
                    nonlocal_correction(tt,:)=nonlocal_correction(tt,:) &
                        -lc_var3d_nc(ii_w0,jj_w0,ii_nc,jj_nc,tt,lc_local_nc_index,:)*weighting_nc(ii_w,jj_w,tt,:)*weighting_nc(ii_w0,jj_w0,tt,:) &
                        -lc_var3d_nc(ii_w,jj_w,i_nc,j_nc,tt,lc_local_nc_index,:)*weighting_nc(ii_w0,jj_w0,tt,:)*weighting_nc(ii_w,jj_w,tt,:)
            ...
            !do source_index=1,n_source_index
            subgrid(i,j,tt,emep_nonlocal_subgrid_index,:,emep_subsource)=subgrid(i,j,tt,emep_nonlocal_subgrid_index,:,emep_subsource)+nonlocal_correction(tt,:)
            subgrid(i,j,tt,emep_local_subgrid_index,:,emep_subsource)=subgrid(i,j,tt,emep_local_subgrid_index,:,emep_subsource)-nonlocal_correction(tt,:)
            !enddo
            ...
            area_weighting_nc(i,j,:,:,tt,:)=weighting_nc(:,:,tt,:)
            
            !For diagnostics only
            nonlocal_correction_average=nonlocal_correction_average+nonlocal_correction(tt,:)
    ...
    if (EMEP_grid_interpolation_flag.gt.1) then
        ...
        write(unit_logfile,'(A,2i)')'Calculating EMEP local subgrid contribution using moving window interpolation method ',EMEP_grid_interpolation_flag
        ...
        allocate (total_weighting_nc(dim_length_nc(x_dim_nc_index),dim_length_nc(y_dim_nc_index),dim_length_nc(time_dim_nc_index),n_source_index)) !EMEP grid weighting for interpolation
        ...
        allocate (proxy_weighting_nc(n_weight,n_weight,dim_length_nc(time_dim_nc_index),n_source_index)) !EMEP grid weighting for interpolation
        ...
        if (EMEP_grid_interpolation_flag.eq.2) then
            allocate (weighting_subgrid(emission_max_subgrid_dim(x_dim_index),emission_max_subgrid_dim(y_dim_index),subgrid_dim(t_dim_index),n_source_index))
            allocate (crossreference_weighting_to_emep_subgrid(emission_max_subgrid_dim(x_dim_index),emission_max_subgrid_dim(y_dim_index),2,n_source_index))
            weighting_subgrid(:,:,:,:)=sum(emission_subgrid(:,:,:,:,:),5)
        ...
        if (EMEP_grid_interpolation_flag.eq.3) then
            ...
            allocate (weighting_subgrid(integral_subgrid_dim(x_dim_index),integral_subgrid_dim(y_dim_index),integral_subgrid_dim(t_dim_index),n_source_index))
            allocate (crossreference_weighting_to_emep_subgrid(integral_subgrid_dim(x_dim_index),integral_subgrid_dim(y_dim_index),2,n_source_index))
            ...
            do i=1,emission_subgrid_dim(x_dim_index,i_source)
                ...
                weighting_subgrid(i_cross,j_cross,:,i_source)=weighting_subgrid(i_cross,j_cross,:,i_source)+sum(emission_subgrid(i,j,:,i_source,:),2)
            enddo
            enddo
            endif
            enddo
        ...
        if (EMEP_grid_interpolation_flag.eq.4) then
            ...
            do i_source=1,n_source_index
                ...
                weighting_subgrid_dim(:,i_source)=integral_subgrid_dim(1:2)
            enddo
            weighting_subgrid(:,:,:,:)=sum(integral_subgrid(:,:,:,:,:),5)
        ...
        do i=1,weighting_subgrid_dim(x_dim_index,i_source)
            ...
            total_weighting_nc(i_nc,j_nc,:,i_source)=total_weighting_nc(i_nc,j_nc,:,i_source)+weighting_subgrid(i,j,:,i_source)
        ...
        if (use_subgrid(i,j,allsource_index)) then
                ...
                if (use_downwind_position_flag.and.hourly_calculations) then
                    ...
                    do tt=t_start,t_end
                        ...
                        do ii=i_start,i_end                
                            ...
                            if (xpos_emission_subgrid.ge.lon_area_min.and.xpos_emission_subgrid.le.lon_area_max &
                                ...
                                proxy_weighting_nc(i_nc-i_nc_c+i_w_c,j_nc-j_nc_c+j_w_c,tt,i_source)=proxy_weighting_nc(i_nc-i_nc_c+i_w_c,j_nc-j_nc_c+j_w_c,tt,i_source)+weighting_subgrid(ii,jj,tt,i_source)
                    ...
                    do ii=i_start,i_end                
                        if (abs(xpos_subgrid-xpos_emission_subgrid).le.lon_limit &
                            ...
                            proxy_weighting_nc(i_nc-i_nc_c+i_w_c,j_nc-j_nc_c+j_w_c,:,i_source)=proxy_weighting_nc(i_nc-i_nc_c+i_w_c,j_nc-j_nc_c+j_w_c,:,i_source)+weighting_subgrid(ii,jj,:,i_source)
                        endif                   
                    enddo
                    enddo
            ...
            do tt=t_start,t_end
            !do i_source=1,n_source_index
                if (total_weighting_nc(ii,jj,tt,i_source).ne.0.) then
                    !proxy_weighting_nc(ii,jj,tt,i_source)=proxy_weighting_nc(ii,jj,tt,i_source)/total_weighting_nc(ii,jj,tt,i_source)
                    proxy_weighting_nc(ii-i_nc+i_w_c,jj-j_nc+j_w_c,tt,i_source)=proxy_weighting_nc(ii-i_nc+i_w_c,jj-j_nc+j_w_c,tt,i_source)/total_weighting_nc(ii,jj,tt,i_source)/EMEP_grid_interpolation_size_sqr
                else
                    !proxy_weighting_nc(ii,jj,tt,i_source)=0.
                    proxy_weighting_nc(ii-i_nc+i_w_c,jj-j_nc+j_w_c,tt,i_source)=0.
                endif
            !enddo
                if (proxy_weighting_nc(ii-i_nc+i_w_c,jj-j_nc+j_w_c,tt,i_source).gt.1) write(*,'(A,7i6,f12.2)')'WEIGHTING>1: ',tt,i,j,ii,jj,ii-i_nc,jj-j_nc,proxy_weighting_nc(ii-i_nc+i_w_c,jj-j_nc+j_w_c,tt,i_source)
            enddo
            ...
            !if (hourly_calculations) then
                ...
                do ii=i_nc_start,i_nc_end
                    ...
                    subgrid(i,j,:,emep_local_subgrid_index,i_source,emep_subsource)=subgrid(i,j,:,emep_local_subgrid_index,i_source,emep_subsource) &
                        +var3d_nc(ii,jj,:,local_nc_index,i_source)*proxy_weighting_nc(ii-i_nc+i_w_c,jj-j_nc+j_w_c,:,i_source)
                    !if (weighting_nc(ii,jj,traffic_index).ne.0) then
           ...
           ! endif
            ...
            nonlocal_correction(:,i_source)=0.
            !if (2.eq.1) then
            ...
            do ii=-1-floor(EMEP_grid_interpolation_size*0.5),+1+floor(EMEP_grid_interpolation_size*0.5)
                ...
                if (jj.ne.0.or.ii.ne.0) then
                    !First weight is emission, the second is area
                    nonlocal_correction(:,i_source)=nonlocal_correction(:,i_source) &
                        -lc_var3d_nc(ii_w0,jj_w0,ii_nc,jj_nc,:,lc_local_nc_index,i_source)*proxy_weighting_nc(ii+i_w_c,jj+j_w_c,:,i_source)*area_weighting_nc(i,j,ii_w0,jj_w0,:,i_source) &
                        -lc_var3d_nc(ii_w,jj_w,i_nc,j_nc,:,lc_local_nc_index,i_source)*proxy_weighting_nc(ii+i_w_c,jj+j_w_c,:,i_source)*area_weighting_nc(i,j,ii_w,jj_w,:,i_source)
            ...
            subgrid(i,j,:,emep_nonlocal_subgrid_index,i_source,emep_subsource)=subgrid(i,j,:,emep_nonlocal_subgrid_index,i_source,emep_subsource)+nonlocal_correction(:,i_source)
            subgrid(i,j,:,emep_local_subgrid_index,i_source,emep_subsource)=subgrid(i,j,:,emep_local_subgrid_index,i_source,emep_subsource)-nonlocal_correction(:,i_source)            
            subgrid(i,j,:,emep_subgrid_index,i_source,emep_subsource)=subgrid(i,j,:,emep_nonlocal_subgrid_index,i_source,emep_subsource)+subgrid(i,j,:,emep_local_subgrid_index,i_source,emep_subsource)
            
            !Averaged over time for diagnostic purposes only
            nonlocal_correction_average(i_source)=nonlocal_correction_average(i_source)+sum(nonlocal_correction(:,i_source),1)/subgrid_dim(t_dim_index)
        ...
        nonlocal_correction_average=nonlocal_correction_average/subgrid_dim(x_dim_index)/subgrid_dim(y_dim_index)
